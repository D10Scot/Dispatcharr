...
`.claude/settings.json` registers `PostToolUse` on `Write|Edit`, each check scoped to the file just edited. Blocking: `*tests/test_*.py` (runs the whole package), `frontend/**/*.test.jsx` (vitest), `*/models.py` (`makemigrations --check`), `live_proxy/{constants,redis_keys}.py` (`manage.py check`), `.github/workflows/*.yml` + `action.yml` + `dependabot.yml` (zizmor), `e2e/**/*.ts` + `e2e-upstream/**/*.ts` (`tsc --noEmit` for that package). Advisory: eslint on `frontend/**/*.jsx`, credential-logging grep on any `*.py`.
...
- The e2e typecheck **blocks**, unlike eslint directly above it, and for the same reason zizmor does: both e2e packages typecheck clean today, so it is a ratchet with nothing to tolerate. It is also the only automated check those trees have — nothing else in the hook matches `*.ts`, and the pre-commit gate routes only backend labels and `frontend/`. A full Playwright run is far too slow for a hook; a typecheck is about a second.
...
Coverage is 45.6% backend / 71.9% frontend and **inversely correlated with criticality**: `hls_proxy` 0%, `plugins` 25%, `vod_proxy` 35.6%, `live_proxy` 38.5% vs `timeshift` 78.7%; `log_parsers.py`, which decides a stream is buffering, is 20.4%. **There is now an end-to-end suite** (`e2e/`, Playwright, five projects: `pristine`, `seeded`, `streaming`, `streaming-failover`, `streaming-greybox`; each is its own CI job in `e2e-tests.yml` and needs its own container). It drives a real AIO container against a fake upstream provider (`e2e-upstream/`) with eight injectable faults. G4 covers the live streaming data path: byte-level TS alignment and continuity, multi-client upstream sharing, mid-stream switching, all three failover triggers, the three Stream Profile architectures, and Output Profile process sharing — **including the repository's first tests that spawn a subprocess**, so ffmpeg lifecycle no longer runs only against hand-written strings. Read `e2e/README.md` before adding to it, and `e2e/COVERAGE.md` for what is and is not covered; the latter is the shared worklist across the seven-goal programme and is updated in the same PR as the tests. `LiveServerTestCase` is still unused, and the backend unit suites are unchanged. Exactly one test file talks to a real Redis; the lease and ring buffer never meet real Redis semantics, and the fakes reimplement the Lua in Python — proving the reimplementation correct while saying nothing about atomicity. Nothing gates on any of it: `ci.yml`, `release.yml` and `docker-build.yml` have no test job in their `needs:` graph, and upstream has no branch protection. (`docker-build.yml` runs on every push to `main` and its `build-and-push` job succeeds, publishing `ghcr.io/d10scot/dispatcharr:latest`; `release.yml` genuinely has never run — no releases exist and tags stop at `v0.29.0`, inherited from upstream.) Test filenames name defects, not capabilities (`test_ts_proxy_ghost_clients`, `test_xc_empty_fetch_guard`) — a free hazard map.

**The E2E suite quarantines its Redis coupling on purpose.** `e2e/fixtures/greybox/redis.ts` is the only sanctioned way a test reaches Redis, and `tests/streaming-greybox/quarantine.spec.ts` enforces an allowlist of importers with a test that fails naming the offending file — not a convention, an assertion. Phase 3 of this fork's direction removes Redis from the video data path, at which point every test in that project must be rewritten or deleted; keeping the coupling in one file makes that a single grep. If you are about to import it from outside `tests/streaming-greybox/`, you are adding work to that refactor. One row is deliberately **not** covered: the un-fenced ownership lease could not be provoked from outside the container — the owning worker's own cleanup loop re-acquires the deleted key with the same worker id in under 500ms, and a follower only contends when channel *metadata* is absent too. `COVERAGE.md` carries the full trace and the untried lever. It is recorded as a gap rather than shipped as a passing test, and that distinction is deliberate.
