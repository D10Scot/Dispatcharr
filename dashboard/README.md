...
- `explore.html` — every catalogued metric, per-commit (default) or daily
  ...
  render daily regardless of the toggle. Milestone lines from `milestones.yml`.
...
(`.venv/bin/python` needs `metrics/requirements.txt` installed; plain
`python -m metrics.build` works too as long as PyYAML is on that
interpreter.)
