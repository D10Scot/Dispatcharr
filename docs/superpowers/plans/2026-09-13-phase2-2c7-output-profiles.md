...
Applied as a diff for Appendix R's reason. A7 goes after A6.6 and before `## Stage 2d`; the Done-log entry goes after the last existing `| 2c-6 ` row.

**The Done-log entry is a TABLE ROW, not a bullet**, and an earlier draft got that wrong: it appended `- **2c-7 — …**`, which after a Markdown table renders as a detached list item, drops the branch and status columns the other nine rows carry, and spells the separator with an em-dash where the table uses `--`. The row here matches 2c-6's cell for cell — three cells, a backticked branch, `pending` — verified against all nine existing rows.
...
index d0f95a25..cdf17664 100644
...
+| 2c-7 -- the Go relay's Output Profiles (`migration/phase2c-output-profile`). One transcode per active `(channel, profile)` pair reading the channel's shared ring on `pipe:0` and writing a second in-process MPEG-TS ring, shared by every client on that profile; an fMP4 client on a profile runs it and 2c-6's remux chained, under `mpegts:p<id>` and `fmp4:p<id>`. Parity-matrix row 11 gets its Go pin, counted in spawns. The contract gained a null `argv` so a broken Output Profile can be told from a deactivated one. Amendment A7. | `migration/phase2c-output-profile` | pending |
