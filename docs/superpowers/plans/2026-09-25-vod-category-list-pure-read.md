...
3. **In `create()` the helper runs before `refresh_categories`.** The pair needs nothing from the provider, so it is created whether or not the provider call that follows succeeds. Pinned by `test_the_pair_is_created_before_the_provider_is_asked` (Appendix B), which makes `refresh_categories` raise; no `DATABASES` entry in `dispatcharr/settings.py` or `dispatcharr/settings_test.py` sets `ATOMIC_REQUESTS`, so Django's per-database default (`False`) applies and the account and the pair survive the exception. (The top-level `ATOMIC_REQUESTS = False` at `dispatcharr/settings.py:162` is inert: Django reads the option only from `DATABASES[alias]`.)
...
- Round 3 (review of `f5f697e0`): the fresh-flag test also sets `auto_enable_new_groups_series: False`; red, green, break-check 3 and `apps.m3u.tests` (250) re-measured, and a probe helper reading `_series` for the series relation passed all 10 new tests. Appendix B round-tripped again.
