...
**The orchestrator fills `<2C7_MERGED_SHA>` when 2c-7 merges, and this plan is re-seeded from that SHA — every `_test.go` included — and every appendix re-verified byte for byte, before it is reviewed.** Until then, Task 0 is the diff between what this plan expects of 2c-7 and what merged.
...
| `internal/relaytest.ControlPlane` with `NewControlPlane`, `RequestsTo`, `Events`, `EventsOfType`, `SetSettings`, `recordEvents` and `nextSourceAnswer` | Task 2 adds `Nameless`, Task 6 adds the authorize route with `AuthorizeDecision`/`SetAuthorize`/`AuthorizeRequests`, Task 4 adds `ClientID` to `RecordedEvent` | a fake whose default arm no longer answers next-source for an unmatched path is fine; the authorize arm is matched by suffix before it |
...
- [ ] **Step 1: Apply Appendix AM — a Go pin on thirteen rows**

Amendment A2.2: a Go pin is a reference appended to the existing `Pin` cell, **not a sixth column**. Rows 26 and 27 take none — the guard requires their cell to be exactly `white-box-only`. If the hunk is stale and you are redoing this by hand, read the HTML comment at the top of that file first: one row is one line, cells are never padded, and no Markdown formatter may be run over it.

**Ten of the thirteen are authorize-matrix rows, and what the Go pin asserts there is narrower than what the row is about.** Those decisions are Django's and stay Django's; the relay's whole share is to ask completely and obey exactly, and that is what the named tests pin — the question travelling in full (rows 16, 20, 22, 23), the internal-principal question travelling as a body field (19), the answer's values being used rather than the path and the socket (24), the denial's status reaching the viewer as itself (25, 30), and the XC path authorizing once (15, 21). Say so in the PR description rather than letting a reader infer that a Go test now decides who may watch what.
...
index 4b82eb7e..6cb1b4af 100644
...
index d0f95a25..8f0cf532 100644
...
@@ -2367,6 +2367,122 @@ a Go test that hands the ring a finite asset once has to make the asset long
...
@@ -2692,6 +2808,8 @@ Filled in as PRs merge; this spec lands as its own PR 0.
