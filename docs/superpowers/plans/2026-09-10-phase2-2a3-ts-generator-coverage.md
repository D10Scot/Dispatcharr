...
| 13 | `ClientSetTests::…` (registration) **and** `GhostClientTests::test_a_client_whose_last_active_goes_stale_is_removed` | Registration: a second tune carrying an already-registered `X-Relay-Client` is answered **503 `{"error": "Failed to register client"}`** and `client_count` stays 3. Ghost: with the heartbeat interval at 1 s and `GHOST_CLIENT_MULTIPLIER` at **2.0** (the shipped value; an earlier draft's 0.1 put the ghost timeout under the generator's own 1 s stats-write throttle and passed against a healthy stand-in for the wrong reason -- see Task 3's "load-bearing" note), a client whose upstream has gone silent is dropped in ≈ 2-3 s — inside the 4 s budget the test asserts, and far outside the only other disconnect path (`stream_timeout + failover_grace_period` = 40 s, which the test asserts is larger). |
