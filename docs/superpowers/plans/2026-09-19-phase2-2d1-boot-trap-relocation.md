...
- **It is what keeps Gate 2's `modules=` from moving** (R4). A shim carries exactly **one** statement under coverage's parser — measured with `coverage.parser.PythonParser` — which is enough that `skip_empty = True` cannot drop the file from the report.
...
Re-derived at the seed with A10.3's own grep, narrowed to the four names:

```bash
grep -rn --include='*.py' -E "^[[:space:]]*from [A-Za-z_. ]*(redis_keys|constants|config_helper) import .*(RedisKeys|ChannelMetadataField|ChannelState|ConfigHelper)" . \
...
The read has not gone anywhere — it is at `apps/proxy/config_helper.py:50` — it has left the scope the SITES list describes. SITES goes **12 → 11**.
...
```
Hit(path='apps/proxy/config_helper.py', lineno=50, symbol='get_proxy_settings', shape='model_method')
```
...
**The prose that cites the deleted SITE is updated, not left dangling.** `zero_orm_allowlist.py:439`, `:629`, `:636`, `:639`, `:665` and `:672` mention "config_helper.py:50" inside comments and `reason` strings. Nothing asserts on them, but a comment citing an entry this PR deleted is exactly the stale-citation shape this programme keeps finding. Appendix E's hunk carries the one that matters — the re-pointed EdgeEntry's own `reason`, which explains the whole move in place — and Task 4 Step 3 is the sweep for the rest.
...
Expected: `16fbb952942a64115928e8d0ff8dacce3d0eb976`. If it is not, `origin/main` has moved past the seed — **stop and report** rather than guessing what came in between. Every count, hash and line number below was measured on that commit.
...
Expected, in order: `redis_keys.py` and `constants.py` print `(no module-level imports)`; `config_helper.py` prints `5:from apps.proxy.config import TSConfig as Config`; and the three `ls` targets all report **No such file or directory**. A pre-existing `apps/proxy/constants.py` is a stop-and-report — R1's no-collision claim would be false.
...
grep -rn --include='*.py' -E "^[[:space:]]*from [A-Za-z_. ]*(redis_keys|constants|config_helper) import .*(RedisKeys|ChannelMetadataField|ChannelState|ConfigHelper)" . | wc -l
echo -n "19? outside the package: "
grep -rn --include='*.py' -E "^[[:space:]]*from [A-Za-z_. ]*(redis_keys|constants|config_helper) import .*(RedisKeys|ChannelMetadataField|ChannelState|ConfigHelper)" . \
  | grep -v "/apps/proxy/live_proxy/" | grep -v "apps/proxy/tests/test_stream_limits.py" \
  | grep -v "apps/channels/tests/" | grep -v "apps/proxy/tests/test_stream_switch.py" \
  | grep -v "apps/proxy/tests/test_boundary_error_arms.py" | wc -l
...
Expected: `140`, `19`, `2`, `29`. A different 140 or 19 means R3's and R5's tables need re-deriving before anything is edited — **stop and report** with the actual list. (`apps/timeshift/redis_keys.py`'s `TimeshiftRedisKeys as RedisKeys` imports are what the `test_stream_limits.py` exclusion removes; they are a different class with a colliding alias.)
...
`apps/proxy/redis_keys.py` and `apps/proxy/config_helper.py` are their originals with the module docstring replaced and **one byte trimmed**: the seed files end with a blank line before EOF, which `git apply --whitespace=error` rejects in a newly added file. `apps/proxy/constants.py` carries `ChannelState` and `ChannelMetadataField` verbatim and nothing else (R2).
...
Expected: `both leaves: OK`; `config_helper.py` prints exactly one module-level import, `5:from apps.proxy.config import TSConfig as Config`; and the statement counts are **85**, **64**, **73**. A different count means the appendix was edited on the way in.
...
Expected: both `identical` lines, modulo the single trailing blank line Step 1 mentions (`diff` reports `\ No newline` differences, not content ones; if it reports a content difference, **stop**). This is the check that the move is a move.
...
Expected: `1`, `25`, `1`, total **27**. The `1`s are load-bearing: a zero-statement file would be dropped by the rcfile's `skip_empty = True` and R4's `modules=` claim would fail.

- [ ] **Step 3: Confirm the package still imports**

```bash
dx -c "import apps.proxy.live_proxy.server" 2>&1 | tail -3 ; echo "exit=$?"
dx manage.py check 2>&1 | grep -E "System check|Error" | head -3
```

Expected: no traceback, and `System check identified 1 issue (0 silenced)` — the same `staticfiles.W004` as Task 0 Step 6. Anything else is a **stop**.
...
Every hunk is a single line changing `apps.proxy.live_proxy.<module>` to `apps.proxy.<module>`. Nineteen of them, in ten files.
...
Expected: exactly **seven files, eight lines** — `apps/proxy/tests/test_stream_switch.py:12` and `:13`, `apps/proxy/tests/test_boundary_error_arms.py:15`, `apps/channels/tests/test_ts_proxy_teardown.py:7` and `:10`, `apps/channels/tests/test_get_stream_assignment.py:9` and `:10`, `apps/channels/tests/test_ts_proxy_initializing.py:14` and `:15`, `apps/channels/tests/test_channel_stream_reuse.py:15`, `apps/channels/tests/test_ts_proxy_ghost_clients.py:15` and `:16`. (That is twelve lines across seven files; the eight-vs-twelve arithmetic is why you read the list rather than the count.) **Nothing under `apps/timeshift/` and nothing outside `*/tests/`.** A timeshift hit means R6's five lines were missed; a non-test hit means one of R5's nineteen was.
...
Expected after Appendix E: the hits are at the re-pointed `EdgeEntry`'s own `reason` (which Appendix E writes, and which says the read is now at `apps/proxy/config_helper.py:50`) and in four `SQL_SIGNATURES` comments/reasons that still say `config_helper.py:50`. **Change those four to `apps/proxy/config_helper.py:50`** — the read did not move lines, only files, so the line number is still right and only the path needs qualifying. Nothing asserts on these strings; they are corrected because a comment citing a deleted allowlist entry is the stale-citation shape this programme keeps finding. Do not rewrite their substance.
...
Expected: `SITES 11 EDGES 12`, `scanner sites 11`, and `hits 1` with the single hit `Hit(path='apps/proxy/config_helper.py', lineno=50, symbol='get_proxy_settings', shape='model_method')`.
...
**If the digest is not `8cb5c65dac3e`, stop.** The likely cause is a shim that lost its statement (a zero-statement file is dropped by `skip_empty = True`) or a new module accidentally placed under `apps/proxy/live_proxy/`. Do not re-baseline; diff the printed file list against `scripts/coverage_live_path.floor.modules`.
...
Measured on this PR's exact shape. **Do not act on that last line.** R4 says why: `--write-floor` writes one local run's figure where the policy is the worst of ≥12 CI rounds, and a drop bought by removing statements from the denominator is not a coverage improvement to ratchet on. 2d-5 is where `missing` moves.
...
```bash
cd /Users/dion/git/Dispatcharr/.worktrees/phase2d-boot-trap-relocation
grep -n "shape-only" docs/superpowers/specs/2026-09-09-phase2-go-relay-design.md
grep -n "thirteen test files\|thirteen surviving test\|(2 files): \`test_views.py\`" docs/superpowers/specs/2026-09-09-phase2-go-relay-design.md
grep -c "^#### Amendment A11" docs/superpowers/specs/2026-09-09-phase2-go-relay-design.md
```

Expected: every surviving `shape-only` mention attributes the re-baseline to **2d-4** or describes the floor's general procedure — **none** assigns one to 2d-1; the A10.14 bullet now carries the A11.4 sentence; and exactly `1` A11 heading.
...
old paths remain as shims carrying one statement each — enough that `skip_empty` cannot
drop them. Measured at the seed and on this branch with a no-op `coverage run` under the
same rcfile (`source =` triggers coverage's unexecuted-file walk, so the set does not
...
`apps/proxy/config_helper.py:50`, but scope 1 is the package only, so the ratchet reports
...
- All five routed backend labels green: 105 / 349 / 403 / 441 / 429 (+1 skip).
...
index cde6c037..61fb7143 100644
...
+            "apps/proxy/config_helper.py:50's TSConfig.get_proxy_settings(), "
+            "which was config_helper.py:50's SITE entry before the move put it "
+            "outside scope 1 -- but scan_edge() is transitive WITHIN THE MODULE "
...
index 08c861ef..2ffb4b7a 100644
...
@@ -2996,18 +2996,26 @@ census that makes the survivor gate real cannot be taken until after the delete.
...
+therefore changes. 2d-1 leaves the OLD paths in place as re-export shims, each carrying one statement
+so `skip_empty` cannot drop it, and the 38-file set is unchanged — measured at `16fbb952` by running
...
+removes statements from the denominator and the one-statement shims that replace them execute on
...
@@ -3424,7 +3432,12 @@ and each errors at import and fails its whole label.** Re-derived at this SHA by
...
@@ -3538,7 +3551,10 @@ planner can decline them by citation rather than by judgement.
...
@@ -3560,6 +3576,117 @@ planner can decline them by citation rather than by judgement.
...
+module docstring plus one `from … import X  # noqa: F401`, which is **one** statement under
+coverage's parser — enough that `skip_empty = True` cannot drop the file from the report.
+
...
+`apps/proxy/config_helper.py:50`, but scope 1 is `apps/proxy/live_proxy/**` only, so the scanner
...
@@ -3783,10 +3910,11 @@ share a PR with the delete that makes it measurable).
...
@@ -4004,6 +4132,7 @@ Filled in as PRs merge; this spec lands as its own PR 0.
