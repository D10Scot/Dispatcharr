...
- **`CLAUDE.md`.** A rewrites six bullets in § Known defects (lines 128, 129, 130, 134, 135, 136) into one-sentence closure records, the section's precedent for #295 and #190, and rewrites one clause each in line 63 (§ Test hooks) and line 82 (§ Architecture › State). **J** edits the `hls_proxy` and dead-`RedisKeys` sentences (§ Architecture, § Structural constraints, the "Dead or unwired" paragraph after line 136, § Testing). **E** (#232) edits the "Two debugging traps" paragraph. No sentence is shared. The one proximity risk is J's "Dead or unwired" paragraph two lines below A's last deleted bullet; whichever lands second rebases, and A's edits replace whole bullets, so the resolution is mechanical. **B goes first and edits none of A's lines.**
...
- **`relay/httpapi/stream.go:392-395` and `relay/httpapi/fanout_test.go:443-449`: J-1, comments only** (J's plan, files table line 44 and its Steps 1-2, which re-word the `hls_proxy` sentence in each). A edits neither file. But the matrix cites `stream.go` below J-1's edit: rows 17 (`:558-570`) and 8 (`:940-960`) at the seed, and row 12 (`:1005-1031`) once A-5 rewrites it. So if J-1's rewording changes the comment's line count, those three ranges shift. **A lands first** in the programme order, so J re-anchors them (J's own overlap row says "A first"). If the lead re-seeds A-5 after J-1 has merged, A-5's implementer re-anchors row 12's `stream.go` range on the quoted code instead (A-5 Task 3 says so). Found by the round-2 review.
- **`docs/relay-parity-matrix.md`, the rest of `relay/**`, `core/relay_events.py`, `apps/proxy/relay_serializers.py`.** A only.
...
**If the capture cannot be taken at all** (no Docker, no network, the apt mirror down), **STOP and report.** Do not write a capture by hand, do not synthesise one from the 8.1.2 files, and do not commit the two new `CorpusNames` entries without both files. `TestReadStderrSplitsOnCROrLFAndSeesEveryRecord` iterates `CorpusNames` (`spawn_test.go:75`), so the package would not even run. #227 and #24 can proceed in the meantime only if the lead splits #299 out of A-1.
...
- **`CLAUDE.md`**: replace the #299 bullet (seed line 135) with a one-sentence closure record, the section's precedent for a closed defect (#295, #190): "- **The buffering-progress gate was blind on ffmpeg 6.x** ([#299](https://github.com/D10Scot/Dispatcharr/issues/299)): stream-copy records there begin `size=` and carry no `frame=`, and `relay/ffmpeg/progress.go`'s `IsProgressLine` now accepts them, pinned by two real ffmpeg 6.1.1 captures (fixed in #<PR>)." In line 63, replace "so the verbatim-ported `IsProgressLine` gate never passes and the pin fails rather than skips when `CI` is set" with "which the `IsProgressLine` gate did not accept until [#299](https://github.com/D10Scot/Dispatcharr/issues/299) was fixed, so the pin failed rather than skipped when `CI` is set". The rest of that sentence (why CI runs on the base image) stays true and stays.
...
- **`CLAUDE.md`**: replace the #221 bullet (line 128) and the #302 bullet (line 129) with one-sentence closure records: "- **`MAX_STREAM_SWITCHES` now bounds buffering-triggered switches** ([#221](https://github.com/D10Scot/Dispatcharr/issues/221), fixed in #<PR>, row 6): once the budget is spent the buffering path refuses the switch and keeps the slow source, where the main loop still ends the channel at its bound." and "- **A buffering timeout with no alternate no longer asks `next-source` on every progress record** ([#302](https://github.com/D10Scot/Dispatcharr/issues/302), fixed in #<PR>): a failed switch defers the next ask by one `buffering_timeout`." In line 82, replace "the buffering-triggered switch bypasses `MAX_STREAM_SWITCHES` as in Python (row 6)" with "the buffering-triggered switch counts against `MAX_STREAM_SWITCHES` and, once it is spent, is refused rather than fatal (row 6, [#221](https://github.com/D10Scot/Dispatcharr/issues/221))".
...
- **`CLAUDE.md`**: replace the #296 bullet (line 134) with a one-sentence closure record: "- **The UDP user-agent filter no longer leaves dangling flags** ([#296](https://github.com/D10Scot/Dispatcharr/issues/296), fixed in #<PR>): a dropped value takes the flag before it and a dropped flag the value after it, which also repairs the shipped Streamlink profile on a UDP upstream."
...
**The unhealthy test's clock** is read *before* the last `Healthy()` call that returned `true`, so the flip, and the cap's clock that cannot start before it, both come after it and the measured gap is never shorter than the real one. The prototype's first draft read it after the poll's 10 ms wait and measured **1.99 s against a 2 s cap once in a full package run under `-race`**: the flip had landed inside the wait. Three repeats of the corrected test, 29.4 s each; most of that is teardown waiting on the control-plane hold, as `TestAClientIsDroppedOnceTheKeepaliveCapIsReached` (18 s) already does. **Budget for it**: it adds about half a minute to every `relay/httpapi` run, and so to GC 3's roughly four-minute commit gate. A quiet half-minute inside that test is the teardown, not a hang, and is not a reason to shorten the hold: below the 6 s assertion window, the failover could end the channel under the test.
...
- **Matrix row 12**, replaced in place:
  ...
  (Prototype span: `fmp4.go:112-239`.) The `relay/httpapi/stream.go:1005-1031` citation is the TS loop's keepalive and client-timeout branches. If J-1 (a comment edit at `stream.go:392-395`) has merged first and changed that comment's line count, re-anchor the range on that code rather than on the number (see § Overlap).
- **`CLAUDE.md`**: replace the #222 bullet (line 130) with a one-sentence closure record: "- **An fMP4 viewer is no longer dropped 40s into a stall a TS viewer survives** ([#222](https://github.com/D10Scot/Dispatcharr/issues/222), fixed in #<PR>, row 12): `serveFMP4Client` now leaves by the TS loop's exit, never on a healthy channel and after `MAX_KEEPALIVE_DURATION` on an unhealthy one, with no keepalive bytes written."
...
- **`CLAUDE.md`**: replace the #314 bullet (line 136) with a one-sentence closure record: "- **`GET /proxy/ts/status/<uuid>` carries `ffmpeg_bitrate`** ([#314](https://github.com/D10Scot/Dispatcharr/issues/314), fixed in #<PR>): the relay renders the output bitrate it parses, and `source_bitrate`, which nothing ever wrote, left the serializer."
