...
A decision rule written after seeing the number is not a decision rule. Task 2 Step 6 fixes the scope from the measured shortfall by the table in that task, and the implementer follows it. The one branch that is *not* the implementer's call is `shortfall > 600`: that means Tier A alone cannot meet the gate, and the remaining options (Tier B harness work, Tier C deletion at 0.8 on the statement, or a spec amendment to the threshold) are decisions above an implementer. **Stop and escalate; do not start writing tests against a target you cannot reach.**
...
- [ ] **Stopping rule:** maximum unchanged for ≥6 consecutive rounds, minimum **12** rounds total. This mirrors `scripts/coverage_live_path.floor`'s own procedure, step 1.
...
- [ ] Fix the scope from the measured shortfall. This rule was written before the number was known (R4) and is followed, not re-argued:

| measured shortfall | scope | est. available | cushion |
|---|---|---:|---:|
| **≤ 300** | Tasks 3, 4, 5, 6, 7, 11 only. Skip Tasks 8, 9, 10. | 245 | tight — add Task 8 if a checkpoint undershoots |
| **301 – 480** | **Tasks 3-11 as written.** This is the expected band. | 488 | 2-62% |
| **481 – 600** | Tasks 3-11, **plus** `views.py`'s `_channel_setup_needed` is already in Task 8; add `stream_ts`'s connection-retry loop (`views.py:348-394` — the `remaining_time <= retry_interval` break, the `gevent.sleep` back-off, the final attempt at the timeout boundary, and `control_plane.release_source` on the abandoned slot) as Task 8b, and `input/manager.py`'s `_wait_for_existing_processes_to_close` (18 statements, **zero** in `except`, pure polling logic) plus the mockable socket/reconnect arms of `_close_socket` and `_attempt_reconnect` as Task 9b. | ~600 | 0-25% |
| **> 600** | **STOP. Escalate to the orchestrator before writing a single test.** Tier A cannot meet the gate from here. The options — Tier B harness work, Tier C deletion at 0.8 on the statement, a spec amendment to the threshold — are above an implementer's call (R4). | — | — |
