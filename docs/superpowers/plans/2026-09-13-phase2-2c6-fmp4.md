...
2c-5's plan is on branch `docs/phase2c5-plan` at `d77f5f74` (`docs/superpowers/plans/2026-09-13-phase2-2c5-failover.md`; read it with `git -C <repo> show "d77f5f74:docs/superpowers/plans/2026-09-13-phase2-2c5-failover.md"` — **brace the expansion in zsh**, or the `:path` is eaten as a history modifier and you get the tip commit's diff instead, exit code 0 and all).

**That branch moved twice while this plan was being written**, `c90efe79` → `9437b6cb` → `d77f5f74`, and every appendix here was re-extracted and the whole module re-verified after each move.

The first move was a re-seed against `d92b33be` plus the `relaytest.Upstream` throttle fix for [#300](https://github.com/D10Scot/Dispatcharr/issues/300); two Go appendices changed and neither touched this ledger. **The second, 2c-5's fix round after its own review, changed fourteen and one of them mattered here: `Channel.Switching()` and its `switching` field are DELETED, and `serveClient`'s `url_switching` exemption branch goes with them** — 2c-5 now states that exemption as not ported, on the ground that the keepalive path shadows it, which is what parity-matrix row 12's own Notes already said. **Row 12's contrast is unaffected**, because it rested on the health gate and not on the exemption: break-check 15 was re-run against the reshaped clause and reddens with the same message. What it changes is prose, in three places, all corrected here — Ruling R7's table, `serveFMP4Client`'s doc comment, and the ledger row below.

After both moves: build, vet and `golangci-lint` at zero issues under native, `GOOS=linux` and `GOOS=darwin`; a full `-race` module run green; row 12's pin green and its break-check still red. The #300 fix makes a throttled upstream deliver its bytes *sooner*, which can only make row 12's `DeadAirAfterBytes` stall arrive earlier; it cannot make that test pass by being slow. The regenerated golden is **byte-identical** to the one 2c-5's plan carries, which is what Ruling R11's "this PR touches no Python file" rests on. Its implementation, `migration/phase2c-fmp4`'s predecessor `migration/phase2c-failover`, was **not merged and not reachable** when this plan was written: no local branch, no remote branch, no worktree carried it.
