...
2c-5's plan is on branch `docs/phase2c5-plan` at `c90efe79` (`docs/superpowers/plans/2026-09-13-phase2-2c5-failover.md`; read it with `git -C <repo> show "c90efe79:docs/superpowers/plans/2026-09-13-phase2-2c5-failover.md"` — **brace the expansion in zsh**, or the `:path` is eaten as a history modifier and you get the tip commit's diff instead, exit code 0 and all). Its implementation, `migration/phase2c-fmp4`'s predecessor `migration/phase2c-failover`, was **not merged and not reachable** when this plan was written: no local branch, no remote branch, no worktree carried it.
...
| `relaytest.Config` with `DeadAirAfterBytes`; `relaytest.ControlPlane.RequestsTo`; `relaytest.EffectiveProxySettings()` carrying `STREAM_TIMEOUT` and `FAILOVER_GRACE_PERIOD` | The row-12 test sets `DeadAirAfterBytes` and overrides those two settings | a missing `DeadAirAfterBytes` is a stop: the row-12 test needs the **upstream** to stall, not the remux (Ruling R7 and break-check 15) |
