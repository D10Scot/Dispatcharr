...
|---|---|---|---|
| `e2e/COVERAGE.md` | four rows: the relay-control-API row, the TTFB row, the bounded-restart row, and the wrapped `:259-261` bullet (its Appendix K) | **five different rows** that cite an `apps/proxy/live_proxy/` **path** (`:190`, `:192`, `:193`, `:202`, `:203` at this seed) | **Line numbers move; no row is edited twice.** Note 2d-3's own replacement text for the relay-control-API row does **not** introduce a `live_proxy` path, and its `:259-261` rewrite removes one Redis assertion and adds no citation. Appendix J anchors on row text, not line numbers. |
| `CLAUDE.md` | six passages (§ Architecture's "no nginx location routes to it", the `uwsgi_buffering off` sentence, § Auth's `auth_request` sentence, § Commands' `all-dev` paragraph, the Phase 1 PR 8 restart paragraph, § Test hooks) | **eleven different passages** (Appendix K) | **Different sentences, same file.** Appendix K is an anchored replacement script with `assert count == 1`, not line-anchored hunks, for exactly this reason — 2d-2's and 2d-3's idiom. |
| the spec | Amendment A13 + eight in-place corrections + a Done-log row | **Amendment A14** + six in-place corrections + a Done-log row (Appendix N) | **Different passages.** A14 anchors on `#### Amendment A13`'s own trailing text, which does not exist at this seed — Task 0 Step 7 verifies it does after the re-seed and STOPs if it does not. |
...
naming the variable and carry on with the default rather than raising. Appendix C carries the diff.
...
### R6 — `dispatcharr/urls.py`'s two `stream_xc` routes are deleted, and `tests/test_urls_xc_three_segment.py` is rewritten rather than deleted

`dispatcharr/urls.py:9` imports `stream_xc` from the deleted package and `:69-78` registers it on
two root-level patterns: `^live/<u>/<p>/<id>` and `^<u>/<p>/<id>`. After 2d-3, nginx serves both from
the Go relay — `location ^~ /live/` (`docker/nginx.conf:377`) and the three-segment regex
`location ~ ^/[^/]+/[^/]+/\d+(?:\.[A-Za-z0-9]+)?$` (`:500`) are two of the three byte-path locations
2d-3 flips. Django never sees either in an nginx-fronted shape. **Ruling: delete the import and both
`re_path`s.**

**What that costs, and the one thing it must not lose.** The two patterns exist in their current
shape because of Phase 1's D7 "three-segment regex trap": `XC_STREAM_ID_PATTERN` narrows the last
segment to digits-with-optional-extension so an SPA deep link of the same shape
(`/settings/example/page`) falls through to the catch-all instead of being swallowed by
`stream_xc`'s `get_object_or_404`, whose `Http404` DRF's exception handler absorbs before Django's
catch-all can see it. `tests/test_urls_xc_three_segment.py` (5 tests, label `tests`) pins it, and
three of its five assertions resolve a path *to* `stream_xc`.

Deleting the patterns makes those three assertions unwritable, but it does **not** make the property
irrelevant — it moves the discrimination entirely into nginx's regex. **Ruling: rewrite the file
rather than delete it.** It keeps `test_non_numeric_three_segment_path_falls_to_spa_catch_all`
(unchanged, and now the whole Django-side property: a three-segment path resolves to the SPA
catch-all), keeps the `redact_url` half of
`test_resolver_and_redact_url_agree_on_credential_shape` — rewritten so the XC-shaped path's
assertion is "`redact_url` still masks it" without also asserting a resolver match that no longer
exists — and **adds** a test asserting that an XC-shaped three-segment path *also* resolves to the
catch-all now, which is the behaviour change this PR makes and the one a future reader will want
named. The docstring is rewritten to say that `docker/nginx.conf`'s
`location ~ ^/[^/]+/[^/]+/\d+…$` is now the only router for the XC live shape, pinned by
`e2e/tests/streaming-greybox/nginx-stream-buffering.spec.ts`. Appendix G carries the file.

`e2e/tests/streaming/spa-three-segment-route.spec.ts` — the black-box half of the same property —
needs **no** edit: it asserts the SPA is served, which is now true for a strictly larger set of
paths. Verified by reading it; `grep -n "stream_xc" e2e/tests/streaming/spa-three-segment-route.spec.ts`
returns nothing.

### R7 — fifteen test files outside the package, per-file
...
import / patch-string / other counts per file. Its output is Appendix A's provenance and Task 0
Step 3 re-runs it.
...
**Net: 3 files deleted whole, 5 split, 6 kept by re-pointing, 1 rewritten.** Test-count effect,
measured by counting `def test_` in each deleted region (Appendix A carries the arithmetic):
...
**So the plan ships a break-check rather than relying on a green run** (Task 4). A new test in
`apps/channels/tests/test_get_stream_assignment.py` seeds **only** the metadata hash — no
`channel_stream:` and no `stream_profile:` key — and asserts `release_stream()` returns `False` and
logs the "no profile found" warning at `models.py:756-759`. That is precisely the behaviour the
delete *changes*, and today nothing observes it in either direction. Its break-check is the
reversion: restore the `:745-750` fallback and the test goes green-when-it-should-be-red, printing
...
gains one sentence, in Appendix H: *the gate is deliberately slack between 2d-4 and 2d-5 — a floor
...
non-`live_proxy` citation already on the row is kept in place. Appendix E carries all 21 cells.
...
topology was built and run in the scratchpad; results in Appendix D's preamble):
...
right provenance anchor); `WriteChunk = 9400` is `TS_PACKET_SIZE * 50`. Appendix D carries both
files.
...
merely describes `live_proxy` as existing is 2d-6's.** Eleven passages, each quoted in Appendix K
...
- **`e2e/`** — 28 hits in 13 files: 22 prose comments, 5 `COVERAGE.md` citations, 1 `e2e-upstream/README.md`
  line. **Zero guard-checked paths, zero test-title pins, zero functional reads.** The five
  `COVERAGE.md` citations are this PR's (Appendix J) because they name a path; the 23 comments are
  2d-6's. No guard reads `COVERAGE.md` (`grep -rn 'COVERAGE' e2e/tests/guards/` → nothing), so all
...
**Deleted (3 paths + one directory of 97 files)**
...
`relay/internal/relaytest/testdata/` — the directory, by the move above. No new source file.
...
| `dispatcharr/urls.py` | the `stream_xc` import and both `re_path`s | R6 |
| `dispatcharr/consumers.py` | one function-local import | R3 |
| `apps/proxy/urls.py` | the `relay/` include and the `ts/` `live_proxy` include | R1, R5 |
...
`apps/proxy/tests/{test_boundary_error_arms, test_combined_stats, test_stream_switch}.py`;
`tests/{test_websocket_consumer_filter, test_ci_test_routing, test_urls_xc_three_segment}.py`;
`core/tests/test_fetch_channel_stats.py` (one docstring). R6, R7.

**Modified — Go (2)**

`relay/internal/relaytest/corpus.go`, `relay/internal/relaytest/asset_test.go`. R12.

**Modified — routing, gates and CI (6)**
...
**Modified — guards, docs and metrics (6)**
...
- [ ] **Step 3 — the reference classifier.** Write Appendix A's script to your scratchpad and run
      it. Its output must match Appendix A's recorded output **line for line**. A new file in the
      list is a reference this plan does not dispose of — STOP. A missing one means 2d-3 or a
      later commit already handled it — STOP and report which.
...
- [ ] **Step 2 — `git apply` Appendix D.1** (`corpus.go`): the header rewrite, `pkgDir()`,
      `RepoRoot()` re-expressed on it, and the one `filepath.Join`.
- [ ] **Step 3 — `git apply` Appendix D.2** (`asset_test.go`): the digest test deleted with its
      reason, the length constant and the two `upstream.py` value pins kept with their derivations.
- [ ] **Step 4 — `git apply` Appendix D.3**: `CAPTURE.md`'s two path mentions and
      `scripts/capture_ffmpeg_stderr.py`'s docstring example.
...
- [ ] **Step 2 — delete the two modules and the include.**
      `git rm apps/proxy/relay_views.py apps/proxy/relay_urls.py`, then `git apply` Appendix C.1
      (`apps/proxy/urls.py`, one line).
- [ ] **Step 3 — `git apply` Appendix C.2**: `internal_base_url.py`'s `dev_url` parameter and the
      `DISPATCHARR_RELAY_GO_PORT` reader, and `relay_client.py`'s call passing it.
- [ ] **Step 4 — `git apply` Appendix C.3**: the three new tests in
      ...
      variable and falls back to 5658. Plus one asserting `get_control_plane_base_url()`'s dev
      branch is **still** 5656, which is the half a careless edit would break silently.
- [ ] **Step 5 — run `apps.proxy.tests`.** `^OK`. It is still collectable at this point because
      Task 7 has not run: `test_boundary_error_arms.py:15` still imports the shim.
- [ ] **Step 6 — the break-check, RUN not read.** Revert `relay_client.py`'s `dev_url=` argument
...
- [ ] **Step 7 — the schema diff.** Re-run `spectacular` to `/tmp/schema-after.yaml` and
...
- [ ] **Step 3 — `git apply` Appendix M.2**: the new test,
      `test_a_metadata_only_assignment_is_no_longer_recoverable`, which seeds **only** the metadata
      hash and asserts `release_stream()` returns `False` and logs the "no profile found" warning.
- [ ] **Step 4 — run `apps.channels.tests`.** `^OK`. (It is still collectable: Task 7 has not run.)
- [ ] **Step 5 — the break-check, RUN not read.** Restore only the `:745-750` fallback read from
      Appendix M.1's reversed hunk and re-run the one test. Expect
      `AssertionError: True is not False` on the return value. Revert. **Record in your report that
      this test is the only thing in the tree that observes these five ranges in either direction**
      — R8's measurement, which is why the plan ships a break-check rather than trusting a green
      suite.
...
- [ ] **Step 1 — `git apply` Appendix C.4**: `dispatcharr/settings.py` (the `INSTALLED_APPS` entry
      and the `fetch-channel-statuses` beat entry), `dispatcharr/urls.py` (the import and both
      `re_path`s), `apps/proxy/urls.py` (the `ts/` `live_proxy` include — the `relay/` one went in
      Task 4), `apps/proxy/apps.py` (`ready()` removed), `apps/proxy/signals.py` (the `live_proxy`
      branch; `hls_proxy`'s kept verbatim).
...
- [ ] **Step 3 — `git apply` Appendix G**: `tests/test_urls_xc_three_segment.py`, rewritten.
- [ ] **Step 4 — verify the beat schedule still parses.** `grep -n "fetch-channel-statuses"
...
- [ ] **Step 2 — delete the four test files R7 disposes of whole.**
      ```
      ...
      ```
- [ ] **Step 3 — `git apply` Appendix A.2**, the seven test-file edits: the three re-pointed
      imports (`test_channel_stream_reuse.py`, `test_get_stream_assignment.py`), the two re-pointed
      patch strings (`test_recording_pipeline.py`), and the four splits
      (`test_ts_proxy_initializing.py`, `test_ts_proxy_keepalive.py`,
      `test_boundary_error_arms.py`, `test_combined_stats.py`, `test_stream_switch.py`), plus
      `core/tests/test_fetch_channel_stats.py`'s one docstring sentence.
- [ ] **Step 4 — re-home the orphaned assertion (R7 #11).** `git apply` Appendix A.3: add
      `width`, `height` and `video_bitrate` to the minimal-channel absence list in
      `relay/httpapi/golden_test.go::TestEveryOptionalFieldIsAbsentRatherThanNull`. This is the one
      assertion of `test_relay_status_shape.py`'s ten that has no cover on either side, and it is
      three strings in an existing list.
...
- [ ] **Step 7 — the break-check for the split files, RUN not read.** For each of the five SPLIT
      ...
      `:15` to `apps.proxy.live_proxy.config_helper` and run the label. Expect
      `ModuleNotFoundError: No module named 'apps.proxy.live_proxy'` and — the part that matters —
      **`Ran 0 tests`**, not one failure: an import error fails collection for the module and the
      label. Paste both lines. Revert.
...
- [ ] **Step 2 — run Appendix E's script** (the matrix). It performs **47** anchored replacements —
      21 Source cells, 26 Pin cells — each with `assert count == 1`, and prints
      `ok, 47 edits`. A different number, or any assertion firing, is a STOP: the anchor it names
      did not match, which means 2d-3 or a later commit moved a row after Task 0 Step 6 measured it.
...
- [ ] **Step 5 — `grep -n "apps/proxy/live_proxy" e2e/COVERAGE.md`** → empty.
      `grep -rn "apps/proxy/live_proxy" e2e/ --include='*.ts'` → still the **23 prose comments**,
      which are 2d-6's (R18). Record the count so 2d-6 has it.
...
- [ ] **Step 2 — run Appendix K's script** (CLAUDE.md). **Eleven** verbatim-string replacements,
      each `assert count == 1`. It prints `ok, 11 edits`. Verbatim-string replacement, not
      line-anchored hunks, because 2d-3 moved every line number in this file (see the re-seed
      table).
- [ ] **Step 3 — verify by replacement count and by grep**, not by eye:
      `grep -c "apps/proxy/live_proxy" CLAUDE.md` → the eleven passages are corrected, but
      **`live_proxy` still appears** in the passages 2d-6 owns. Record the remaining count and the
      line numbers so 2d-6 has its worklist.
- [ ] **Step 4 — run Appendix N's script** (the spec): Amendment **A14** inserted after A13, **six**
      in-place corrections, and the Done-log row. `assert count == 1` on all eight anchors; it
      prints `ok, 8 edits`.
- [ ] **Step 5 — verify the spec carries no contradicting pair.** For each of A14's six
...
- [ ] **Step 5 — `git diff --stat origin/main` names exactly the paths in § File structure**, and
      no others. Count them and say the number in your report.
...
Every `file:line`, every count and every anchor in this plan was opened at the seed
(`dbe56e74`, the tree #327 merges as). What follows is what that pass found and changed, and what
it could not do.

**Verified by running, not by reading.** These are the appendices that were *applied* to a scratch
checkout of the seed during planning, and the result checked:

| Appendix | How it was verified |
|---|---|
| B, C.1, C.2, C.4, L, M.1 | applied, then `python3 -m py_compile` on all nine touched modules — clean |
| D.1–D.3 (the Go corpus move) | applied, then from `relay/`: `go build ./...`, `go vet ./...` and `go test -race` on `./internal/relaytest/`, `./ffmpeg/`, `./drain/`, `./httpapi/`, `./channel/` — **all green**, which is the load-bearing result, since the last two are the cross-package callers a bare relative path would have broken. `golangci-lint run ./...` → `0 issues.` |
| E (the matrix) | run: `ok, 21 Source cells, 26 Pin cells, 2 Notes cells`, and **every one of the 21 rows carried exactly the number of `live_proxy` Source citations this plan predicted**. A re-implementation of the guard's own `CITATION_RE` + range check over the result reports `ALL CITATIONS AND PINS RESOLVE` across all 30 rows. |
| H.1, I.1–I.4, J.1 | applied, then `python -m metrics.build --validate-only` → `ok: 46 metrics, 36 milestones, 32 defects`; `python scripts/ci_backend_test_labels.py scripts/coverage_live_path.floor` → `["apps.channels.tests", "apps.proxy.tests"]`; `go-tests.yml` re-parsed with `yaml.safe_load` → OK |
| A.2 (the test dispositions) | run: `edits: 17`, then `py_compile` on all five split files and a tree-wide grep confirming **no runtime `live_proxy` reference remains** in any surviving test file — only prose comments and `assertLogs("live_proxy")`, which names a logger `apps/proxy/next_source.py:38` still sets |
| G, A.2b (the two rewritten files) | written verbatim, `py_compile` clean |
| K (CLAUDE.md) | **run against the seed**: `ok, 12 edits` — all twelve anchors matched exactly once |
| N (the spec) | **run against the seed**: `ok, 6 edits` — A14, four in-place corrections and the Done-log row, all anchors matched exactly once |

**What could NOT be verified during planning, stated rather than glossed.**

1. **No backend test label was run.** The shared `dispatcharr-testrunner` container is bind-mounted
   at `/Users/dion/git/Dispatcharr/.worktrees/phase2d-admin-wrappers` — the 2d-2 implementer's tree
   — and the edit hook refused every write into the scratch checkout for exactly that reason,
   correctly. Re-pointing it would have been writing into another agent's working state. **So every
   claim in this plan about what a label prints is a prediction the implementer must check**, and
   Tasks 2, 3, 5, 7 and 8 are written to check them rather than to assume them.
2. **`e2e/` has no `node_modules` on this host**, so `npm run typecheck` and
   `npx playwright test --project=guards` could not be run. Appendix F's TypeScript is therefore
   the one appendix whose *compilation* is unverified; Task 10 Steps 3–5 run both, and Step 5's
   four break-checks are what prove the new guard code bites.
3. **The Gate 2 `modules=` hash is a prediction, not a measurement.** `7804792af4f7` was computed
   by hand from the surviving include list using the gate's own formula
   (`sha256("\n".join(sorted(files)))[:12]`), because `--write-floor` needs a real coverage run in a
   container. Task 9 Step 4 compares the printed value against it and **STOPs on a mismatch**
   rather than accepting whichever number appears.

**Corrections this pass made to its own drafts, and to the spec.**

- The test-count table in R7 first said `apps.channels.tests` loses 69 tests; recounted from the
  per-class enumeration it is **100** (13 + 14 + 17 + 4 + 52). Corrected before the rulings were
  written.
- R11 first planned to leave the `[report] include`'s `relay_views.py` line alone; once R1 ruled
  the module out of existence, R9 took both lines, which is why the surviving module list is nine
  and not ten. The spec's PR-5 entry is corrected in place by A14.5 rather than left to contradict.
- A10.14's "eleven files" was taken at face value in the first outline. The classifier found
  **fifteen**, in three kinds, including two files A10.14 does not list and two it lists as
  collection-breakers that are not. A14.3 corrects it in place.
- A10.11's "deletable in 2d-4" was read as covering both `fetch_channel_stats` twins. Reading them
  showed the `core/tasks.py` one is the *migrated* implementation with four tests, and the
  `apps/proxy/tasks.py` one is the Redis-scanning one. A14.2 narrows the sentence.
- The e2e measurement reported `COVERAGE.md` rows `:190`, `:192`, `:193`, `:202`, `:203` as the
  five citing a deleted path. Applying the edit found `:190` and `:192` each carried a **second**
  `views.py` citation further into the row that the first pass missed; all seven mentions across
  four rows are in Appendix J.2, and the one left is `:202`, which A10.10's table assigns to 2d-6.
- Row 15's Source drift (stale since 2b-2, not 2d-2) was found while composing the replacement
  cells and is recorded in A14.7 so a reader reconciling old cells against new does not conclude
  this PR moved something.

**One open question, for the orchestrator rather than the implementer.** 2d-3's Appendix F edits
six CLAUDE.md passages and one of them is in **§ Test hooks**, where this plan's Appendix K edit 1
also lands. The two target different sentences — 2d-3's is the `uwsgi_buffering`/location prose,
this one is the boot-check arm's "three old paths stay in it until 2d-4" — but they are close
enough that the re-seed must check. Task 12 Step 2's `assert count == 1` is the backstop: if 2d-3
rewrote the sentence this plan anchors on, the script stops and names it rather than editing
something else.
...
Every diff below was produced by **applying the edit to a checkout of the seed and running
`git diff`**, not by hand, and the Self-review section above records what was run against each.
They apply with `git apply --check --whitespace=error` at the seed; Task 0 re-seeds after 2d-3
merges, and any appendix that then fails to apply is a STOP, not a thing to hand-merge.

Three appendices are **anchored Python scripts** rather than diffs — E (the matrix), K (CLAUDE.md)
and N (the spec) — in 2d-2's and 2d-3's established idiom, because 2d-3 moves every line number in
the last two and because a diff of twenty-one 600-character table rows is unreadable in review.
Each asserts every anchor matched **exactly once** and prints its own edit count.

### Appendix A.2 — the test-file dispositions (R7)

The eleven surviving test files, after `git rm` of the four R7 deletes whole. It carries the three
re-pointed imports, the two re-pointed patch strings, the five class-deletions, the two rewritten
files (`test_ts_proxy_initializing.py` and `tests/test_urls_xc_three_segment.py`, also reproduced
whole below), `tests/test_ci_test_routing.py`'s two edits, the new `#190` break-check test
(Appendix M.2's other half) and the new `DevRelayAddressTests` (Appendix C.3).

**Applied and verified**: `py_compile` clean on every file, and a tree-wide grep confirming no
runtime `live_proxy` reference remains in any surviving test file.
...
index 2f256eca..5b4c4d1d 100644
...
@@ -213,3 +213,45 @@ class ChannelGetStreamAssignmentTests(TestCase):
             self.assertFalse(
...
+    release_stream() could recover a stream_id and profile_id from that hash
+    when the Django-owned channel_stream:/stream_profile: keys were already
+    gone; after it, it cannot, and returns False.
+
+    NOTHING ELSE IN THE TREE OBSERVES THAT CHANGE. Every other caller of
+    release_stream() seeds the primary keys, which is exactly the branch that
+    skipped all four fallback reads, and no test ever asserted the hdel --
+    the 2d-4 plan's ruling R8 records the audit. So the delete was
+    test-invisible in both directions, which is why this test exists rather
+    than a green suite being taken as evidence.
+    """
+
+    def setUp(self):
...
+        )
+        self.stream = Stream.objects.create(name="s", url="http://e/s", m3u_account=self.account)
...
+
+    def test_a_metadata_only_assignment_is_no_longer_recoverable(self):
+        redis = FakeAssignmentRedis()
+        # ONLY the relay's metadata hash: no channel_stream:, no stream_profile:.
+        redis.hset(
+            RedisKeys.channel_metadata(str(self.channel.uuid)),
+            mapping={
...
+            },
+        )
+        with patch("apps.channels.models.RedisClient.get_client", return_value=redis):
...
+    def test_retry_window_matches_the_relay_timeout_settings(self, _grace, _stream):
...
index e730941a..9f8e6117 100644
...
@@ -172,3 +153,59 @@ class RelayClientStopChannelsGenericExceptionArmTests(SimpleTestCase):
...
+    """Stage 2d-4's dev branch: the two internal directions now differ.
+
...
+    relay since 2d-3; dev runs no nginx, so the Django -> relay direction must
+    name the Go relay's own listener.
+    """
+
+    def setUp(self):
...
index bc9e8395..dae3ea98 100644
...
index dae8a603..02b6a2f6 100644
...
+"""A three-segment path reaches the SPA catch-all, and redact_url still masks
+an Xtream-shaped one.
+
+Until Phase 2 stage 2d-4, dispatcharr/urls.py carried two root-level re_paths
+pointing at apps/proxy/live_proxy/views.py's stream_xc, and this file's subject
+was the "three-segment regex trap" (Phase 1 D7): XC_STREAM_ID_PATTERN narrowed
+the last segment to digits-with-optional-extension so an SPA deep link of the
+same shape fell through to the frontend catch-all instead of being swallowed by
+stream_xc's get_object_or_404, whose Http404 DRF's exception handler absorbs
+before Django's catch-all ever sees it.
+
+Stage 2d-3 pointed nginx's `location ^~ /live/` and
+`location ~ ^/[^/]+/[^/]+/\\d+(?:\\.[A-Za-z0-9]+)?$` at the Go relay, so Django
+stopped seeing either shape in any nginx-fronted deployment; stage 2d-4 deleted
+the view and both patterns. **nginx's regex is now the only router for the XC
+live shape**, pinned by e2e/tests/streaming-greybox/nginx-stream-buffering.spec.ts,
+which asserts the exact set of relay-bound locations.
+
+What is left for Django to guarantee is the half the SPA depends on, and it is
+now unconditional: EVERY three-segment path reaches the catch-all, whatever its
+last segment looks like. The redact_url half is unchanged and still matters --
+dispatcharr/utils.py masks an Xtream-shaped path's credentials wherever one is
+logged, and that is independent of who routes it.
 """
 
 from django.test import SimpleTestCase
@@ -16,36 +30,32 @@ from dispatcharr.utils import redact_url
...
+    def test_an_xc_shaped_three_segment_path_now_falls_to_the_catch_all_too(self):
+        """The behaviour change stage 2d-4 made, asserted rather than implied.
+
+        /user/pass/12345 resolved to stream_xc until this stage. It now
+        reaches the SPA catch-all in Django, because nginx routes the real
+        thing to the Go relay before Django is asked -- and in the one shape
+        with no nginx (DISPATCHARR_ENV=dev) a tune is served from the Go
+        relay's own port, not from here.
+        """
+        for path in ("/user/pass/12345", "/user/pass/12345.ts", "/live/user/pass/12345"):
+            with self.subTest(path=path):
+                match = resolve(path)
+                self.assertIsNone(match.url_name)
+                self.assertEqual(match.kwargs["unused_path"], path.lstrip("/"))
+
+    def test_redact_url_still_masks_an_xc_shaped_path_and_leaves_an_spa_one(self):
+        # dispatcharr.utils' _XC_STREAM_ID_RE is unchanged by 2d-4: a path
+        # shaped like an Xtream credential triple is masked wherever it is
+        # logged, and a same-shaped SPA deep link is not.
...
### Appendix A.2a — the class-deletion script that produced it

Kept because a reviewer should be able to see that no class was deleted by line number:
...
### Appendix B — the three `url_utils` importers and their coupled patch strings (R3)

**One patch on purpose.** R3 explains that splitting the production import from the three patch
strings lets `assert_not_called()` pass vacuously — the "a test can go hollow without changing"
shape, invisible to a diff review.
...
### Appendix C.2 — the `dev` address, both directions (R1)
...
### Appendix C.4 — the boot sites (R4, R5, R6)

`INSTALLED_APPS`, the disabled beat entry, `dispatcharr/urls.py`'s import and both XC `re_path`s,
`ProxyConfig.ready()` and `signals.py`'s `live_proxy` branch. Note `signals.py`'s `hls_proxy`
branch is kept **verbatim** (R17).
...
index b9c42e41..be984298 100644
--- a/dispatcharr/urls.py
+++ b/dispatcharr/urls.py
@@ -6,7 +6,6 @@ from django.views.generic import TemplateView, RedirectView
...
### Appendix D — the Go corpus move and the digest pin (R12)

D.1 `corpus.go`, D.2 `asset_test.go`, in one diff. The four fixture files move by `git mv` in Task 1
Step 1 and carry no content change, so they are not in the diff — `git status` must show four `R`
entries and no `D`+`A` pair.
...
`relay/drain`'s two tests keep working. `CorpusPath` still returns an ABSOLUTE path, which the
...
### Appendix E — the parity matrix's Source and Pin columns (R11)

Run from the repository root. It addresses every row **by id**, asserts each matched exactly once,
and asserts each carried the number of `live_proxy` Source citations this plan measured at the
seed — so a row that moved fails loudly rather than being rewritten from a stale expectation.

**Run at the seed**: `ok, 21 Source cells, 26 Pin cells, 2 Notes cells`, with every predicted
citation count matching.
...
+| 12 | The fMP4 generator's `_is_timeout()` disconnects a client on elapsed time since its last fragment alone — no stream-health check, no `url_switching` exemption and no keepalive — so an fMP4 viewer is dropped `stream_timeout + failover_grace_period` (40s by default) into a stall that leaves a TS viewer on the same channel connected | `relay/httpapi/fmp4.go:110-160`, `relay/httpapi/fmp4.go:222-240` | `relay/httpapi/fmp4_test.go::TestAStalledFMP4ClientIsDroppedWhileATSClientIsNot` | Filed as [#222](https://github.com/D10Scot/Dispatcharr/issues/222) and reproduced, not fixed, per spec D5 (preserve known defects rather than silently fix during the port); also recorded in CLAUDE.md's Known defects. The row's original wording named only the `url_switching` clause; 2a-6 found the TS generator's `_is_timeout()` needs BOTH 40s of no yielded data AND the stream manager unhealthy before it even reaches that clause, and that the keepalive path (below) refreshes `last_yield_time` on every packet it sends whenever a client sits at the buffer head on an unhealthy stream — exactly the condition that would otherwise reach it — so the `url_switching` exemption is practically shadowed by the keepalive mechanism regardless of how long that window itself runs. The reachable divergence is larger: the TS generator disconnects only an unhealthy stream and, before that, sends keepalives that refresh `last_yield_time` for up to `MAX_KEEPALIVE_DURATION` (300s), which the fMP4 generator has no equivalent of. The pin drives the reachable form; the `url_switching` clause is carried in the citations, not tested. This widens the row rather than narrowing it — the original wording described a race a viewer would almost never lose, and the reality is a disconnect on every sustained stall — and it changes the description only: `_is_timeout()` is untouched, D5 still says reproduce rather than fix, and 2c-6 is held to the drop. |
 <!-- block: owed by 2a-5 -->
...
### Appendix F — the `retired:` Source sentinel and its allowlist (R11)

`isRetiredSource` + `RETIRED_SOURCES` in `parity-matrix.ts`, the one-line skip in the citation
test, and the new allowlist test in `WHITE_BOX_ONLY`'s exact three-assertion shape.

**Unverified by compilation** — this host has no `e2e/node_modules`, so `tsc --noEmit` could not
run. Task 10 Steps 3-5 run the typecheck, the guards and the four break-checks.
...
### Appendix G — `tests/test_urls_xc_three_segment.py`, rewritten (R6)

Carried inside Appendix A.2's diff. The whole file also follows, because a reviewer should read a
rewritten pin rather than reconstruct it from a hunk.
...
### Appendix G.1 — the file

Whole file, verbatim — `tests/test_urls_xc_three_segment.py`:
...
### Appendix A.2b — `apps/channels/tests/test_ts_proxy_initializing.py`, rewritten (R7 #5)

Whole file, verbatim — `apps/channels/tests/test_ts_proxy_initializing.py`:
...
+# resolved file set from 38 to 9. THE FLOOR'S `missing` IS DELIBERATELY SLACK
+# UNTIL 2d-5 re-censuses it -- see scripts/coverage_live_path.floor's header.
 [report]
 include =
-    apps/proxy/live_proxy/*
...
### Appendix H.2 — the floor's header (R9)

The `missing`/`modules`/`rcfile` VALUES are written by `--write-floor --shape-only` in Task 9,
never by hand. This is the header paragraph only.
...
### Appendix I.1 and I.2 — routing, the boot-check hook and the isolated script (R10)
...
### Appendix I.3 — `backend-tests.yml`'s coverage matrix (R10)
...
### Appendix I.4 — `go-tests.yml`: the `differential` job (R13)

The job, its `needs` entry, its `env` entry, the success loop, the echo, the header comment's
claim about it, and **both** `live_proxy` clauses in the change detector.

**Verified**: the result re-parses with `yaml.safe_load`.
...
### Appendix J.1 — `metrics/curated/defects.yml` (R14)
...
### Appendix J.2 — `e2e/COVERAGE.md` (R18)

Four rows, seven mentions. `:202` is deliberately left — A10.10's own table assigns it, and
`:50`, to 2d-6, because they describe mechanisms rather than cite paths.
...
### Appendix L — `worker_id` in `ts_admin_views.py` (R2)
...
### Appendix M.1 — #190's five ranges in `apps/channels/models.py` (R8)

The three reads, the two `hdel`s, and `models.py:7`'s now-unused `ChannelMetadataField` import.
`:6`'s `RedisKeys` import stays — twenty-six sites still use it. M.2, the break-check test, is
inside Appendix A.2's diff.
...
### Appendix K — `CLAUDE.md`, the twelve passages this PR makes false (R15)

Takes the measured label count, backend test count and suite wall time as **arguments**, because
every predecessor plan that baked a measured number into an appendix shipped a stale one.

**Run against the seed**: `ok, 12 edits` — every anchor matched exactly once.
...
### Appendix N — Amendment A14, four in-place spec corrections, and the Done-log row

A14 is inserted immediately **before** `## Stage 2d`, so it lands after A13 without needing to
know A13's own text — which does not exist at this plan's seed.

**Run against the seed**: `ok, 6 edits`.
...
A14 = """#### Amendment A14 (stage 2d-4) — eleven rulings from deleting the Python relay
...
three kinds.** A10.14 counts *import statements*. Re-derived with a classifier that separates
...
**A14.4 — the disposition is 3 deleted whole, 5 split, 6 kept by re-pointing, 1 rewritten**, and
...
which joins `relay/httpapi/golden_test.go::TestEveryOptionalFieldIsAbsentRatherThanNull`'s list.
...
DONE_LOG_ROW = """- **2d-4** (`migration/phase2d-delete-live-proxy`) — deleted `apps/proxy/live_proxy/` (97 files,
  ...
  the behaviour they changed; disposed of fifteen outside test files (3 deleted, 5 split, 6
  re-pointed, 1 rewritten); replaced the parity matrix's Python column with Go citations and gave
