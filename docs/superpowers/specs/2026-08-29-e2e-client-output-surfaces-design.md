...
| 11 | Authorization matrix by `user_level` | `seeded` | `output-authorization.spec.ts` | Three channels at `user_level` 0, 1 and 10; XC users at level 0 and 1 plus the admin; assert each principal's `get_live_streams` contains exactly the channels at or below its level **among the three seeded**, and that `stream_xc` returns 404 for one above it |
