...
> **2,003-2,044 missing over 21 rounds → 74.38% at the floor**, against the 1,595 the gate allows.
> **Shortfall: 449 statements.** An independent measurement of the merged tree before 2a-7's own two
...
> one optimistic. Closing 449 statements from where 2a ends means 2-4 further PRs against a pool
...
>
> **D7's stated purpose is met at 74.38%, which is a different claim from its number being met.** The
...
| 2b-4 | `migration/phase2b-coverage-80` | **Closes Gate 2's ≥80% requirement, reassigned here from 2a-7 (§ Stage 2a › Gate 2, amended).** Stage 2a ends measured at **74.38%** — 2,044 missing of 7,978, worst of 21 rounds — with the ratchet floor live in `backend-tests.yml`. This PR closes the remaining **449** statements and moves the floor to the 1,595 the gate allows. Scope it from a *measured* per-file reachability pass, not from an estimate: no reachability estimate in this document survived contact with stage 2a. | The floor file records ≤1,595 missing and `backend-tests.yml`'s `coverage-gate` job is green at it; **D7 blocks every 2c PR until then** | 2a-7 (the ratchet and the measurement it rests on) |
