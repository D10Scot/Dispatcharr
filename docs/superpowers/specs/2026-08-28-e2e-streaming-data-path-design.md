...
**Sibling in flight:** G7 (`2026-08-28-e2e-deployment-lifecycle-design.md`) — disjoint files
...
| D4 | **Grey-box reaches Redis over a published port** — `127.0.0.1:9403:6379` in `scripts/e2e_up.sh` — not `docker exec` | A real client gives typed replies. `docker exec redis-cli` makes every assertion a string-parse. Bound to loopback, matching the existing `127.0.0.1:9191` posture |
