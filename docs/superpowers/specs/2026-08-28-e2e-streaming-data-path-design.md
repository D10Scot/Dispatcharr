...
| `slow-trickle` keeps delivering bytes, at a reduced pacing multiplier | `e2e-upstream/README.md`, fault catalogue | A trickle that keeps delivering makes dead air **impossible**, which is what discriminates a buffering failover from a dead-air one. See D8 |
| **The provider's `ScenarioLog` has no per-chunk granularity.** It records exactly four kinds — `request`, `open`, `close`, `fault` — from four `record()` call sites in the whole source. `close` carries `bytes` and `durationMs` | `e2e-upstream/src/log.ts`; `e2e-upstream/src/server.ts` | A silence check computed over log timestamps measures the CONNECTION'S LIFETIME, not silence, and cannot discriminate. Verified during implementation, after an earlier draft of D8 assumed otherwise. See D8 |
...
| D8 | **The buffering row pre-arms `slow-trickle` and discriminates against dead air by construction** | Pre-arming is mandatory (`speed=` is cumulative). The discriminator is **throughput measured from the `close` event's `bytes` and `durationMs`**, bracketed around the armed trickle rate. Dead air fires only when the provider stops sending; a connection that delivered at approximately the armed rate for its whole life had no >10s silence window, so the watchdog cannot have caused the switch. Bracketed rather than floored because a floor alone would pass a connection that stalled and then burst. **An earlier draft of this decision specified a gap computed over provider log timestamps; that is unimplementable** — the log has no per-chunk entries, so the gap is the connection's lifetime, equally long whether the provider trickled or fell silent, and the row would have been permanently unpassable. `buffering_speed` is raised above 1.0 to arm it in reasonable time |
...
- **The buffering row is the most likely to flake.** It depends on `speed=` crossing a threshold
  ...
  result. Note also that `proxy_settings` is cached process-locally for 10s across four uWSGI
  workers, and saving the row clears that cache only in the worker that handled the write, so a
  threshold change must be given more than 10s to reach the worker that ends up owning the
  channel.
