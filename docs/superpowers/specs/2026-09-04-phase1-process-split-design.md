...
| *(new)* A restart of the control plane does not disturb a running stream. | **Met.** Nothing flushes Redis in any role (D15): `scripts/wait_for_redis.py` becomes wait-only, and AIO's Redis starts empty because it is non-persistent, not because anything wipes it. A modular `web` or `worker` restart therefore leaves a running relay's keys untouched, and PR 8's bounded-restart scenario asserts it. | PR 3 (#173, 9c9100f0), PR 8 (#196, `7685ec50`) |
