...
**Sibling in flight:** G4 (`2026-08-28-e2e-streaming-data-path-design.md`) — **overlapping on six
small surfaces**, not disjoint. Both goals edit `scripts/e2e_up.sh` (G7's D9 adds `--recreate`, G4's
D4 publishes a Redis port — the same `case` block), `e2e/playwright.config.ts` (two new projects
each), `.github/workflows/e2e-tests.yml`'s matrix, `e2e/COVERAGE.md`, `e2e/README.md` and
...
finding in the file, legacy included.
...
**Rulings made at planning time.** Two decisions below were deliberately overridden during
implementation planning, before this text was updated to match — read these first, since the
decisions themselves still describe the pre-ruling shape:

- **G7-R4** overrides **D13**. `provisionAdmin` ships as two exports, `ensureSuperuser`
  (probe/guard/create) and `provisionAdmin` (that, then login), not the single function D13 names —
  a single function would make `bootstrap.setup.ts` spend a login on every run, destroying its
  `reusableTokens` optimisation against the suite-wide 3/minute budget.
- **G7-R6** overrides the "critical" `docker tag` step under Piece A's `upgrade-migrations` bullet.
  `build` produces `dispatcharr-e2e:local` directly — the tag `scripts/e2e_up.sh` already defaults
  to — so no `docker tag dispatcharr-lifecycle:local dispatcharr-e2e:local` step exists; the
  suite jobs still tag into their own names.

See `docs/superpowers/plans/2026-08-28-e2e-deployment-lifecycle.md`: G7-R4 is in that plan's
"Reconciliation with `main`" rulings table; G7-R6 is the "Ruling carried into this task" note under
Task 6. The inline annotations on D13 and the Piece A bullets below point back here.
...
| D3 | **Piece A gets its own workflow, `.github/workflows/lifecycle-tests.yml`. Piece B is split across both workflows: the restart spec joins `e2e-tests.yml`'s matrix, the upgrade spec runs in `lifecycle-tests.yml` — see D16** | Piece A pulls four-plus images and needs an AIO build of its own, so putting it in `e2e-tests.yml` would put a ~60-minute job on every PR. Piece B's two specs then land wherever their runtime allows: the ~3-minute restart spec is a Playwright project like any other, reusing the artifact `e2e-tests.yml` already builds and running in parallel with the other three for ~0 added wall clock, while the ~9-minute upgrade spec does not fit that budget and belongs beside Piece A's build. `e2e/README.md` already obliges a new project to be added to the `e2e-tests.yml` matrix, which is what makes the restart half free |
| D4 | **`lifecycle-tests.yml` triggers on a path-filtered `push` to `main`, a weekly `schedule`, `workflow_dispatch`, and one narrow `pull_request`. `cancel-in-progress: ${{ github.event_name == 'pull_request' }}`** | The post-merge trigger shape is the settled decision from the working ledger, kept as settled. Push filter: `docker/**`, `**/migrations/**`, `scripts/e2e_up.sh`, `scripts/wait_for_redis.py`, `dispatcharr/settings.py`, `e2e/tests/lifecycle/**`, `e2e/fixtures/**`, `e2e/setup/**`, `e2e/playwright.config.ts`, and the workflow file itself. The three `e2e/` additions beyond the test directory are not padding: the lifecycle projects depend on the shared fixtures and on the config, and D13's `provisionAdmin` extraction lands in `e2e/setup/` — a change to any of them can break these specs while touching nothing the filter would otherwise see. `**/migrations/**` is load-bearing under D16: the upgrade job exists to catch a migration that breaks or destroys data, and migrations land under `apps/*/migrations/` and `core/migrations/` — both nested, so the leading `**` has a directory to match. **The narrow `pull_request` trigger** is filtered to `**/migrations/**` *and* `**/models.py`, with the two bash-suite jobs gated off by `if: github.event_name != 'pull_request'`. `models.py` is there because a model change is what *necessitates* a migration, and a PR that alters one without shipping the migration is exactly the class `CLAUDE.md`'s `makemigrations --check` hook exists for — filtering on migrations alone would miss it. **The `build` job must not be gated off**: the upgrade job `needs:` it, and a skipped dependency skips the dependent, which would make every PR run a silent no-op. The **schedule is an addition, and it earns its place**: these suites pull `postgres:16`, `postgres:17`, `redis:latest` and upstream's `:latest` — four floating tags whose contents change under this repository, and which no path filter can ever notice. `cancel-in-progress` is conditional because this workflow is now two things: cancelling a post-merge run drops lifecycle signal for every commit but the last in a run of merges, and those commits are already on `main` — but on a PR the ordinary latency argument applies, and without the conditional three pushes to a migration-touching PR queue three serialised runs, each carrying a 45-minute-budget AIO build. **A path-filtered workflow must not be made a required check**: on a PR that touches none of those paths it never reports, and a required check that never reports blocks the merge forever |
...
| D7 | **Diagnosability is designed, not hoped for.** Each suite runs under `pipefail`, tee'd to a file, and that file is uploaded as an artifact on `always()`. A `failure()`-only step dumps `docker ps -a` and `docker logs` for anything still standing. **`--keep-on-fail` is not passed** | These suites assert via `docker logs \| grep`; a bare red X is useless. Three specifics. (1) `pipefail` is non-negotiable — and it is backwards to assume the default shell provides it. GitHub's default `run:` shell on Linux is `bash -e {0}`: `-e`, but **no** `-o pipefail`. Only an explicit `shell: bash` gets `bash --noprofile --norc -eo pipefail {0}`, so `run: … \| tee …` reports correctly *only if* `shell: bash` is set; without it, `false | tee /dev/null` exits 0 and the job goes **green on failure**. Set `shell: bash` explicitly on every piped step and say why in a comment. (2) `--keep-on-fail` looks like the obvious answer and is a trap, for the two mechanisms in the fact table: cascading name collisions in the TLS suite, and run-global resource accumulation in the puid suite. (3) The suites' own `dump_logs_on_fail` and their closing named-failure list are already in the stream being tee'd — the artifact makes them durable and greppable, and the job summary should name the one-scenario re-run command (`bash docker/tests/test-puid-pgid.sh --skip-build <scenario>`) |
| D8 | **Two new Playwright projects, `lifecycle` (restart) and `lifecycle-upgrade`, sharing one configuration: `workers: 1`, `fullyParallel: false`, `retries: 0`, a 600 000 ms timeout, no `dependencies`, no `storageState`. Each owns its container's lifecycle and must run alone** | Settled decision D-3, made concrete. **Two projects rather than one** because D16 sends the two specs to different workflows, and a split has to be structural to hold: selecting with `--grep` matches *test titles*, so which spec runs would depend on wording nobody has written yet, and nothing would give the `e2e-tests.yml` job the complementary filter — it would run both specs and put the ~9-minute upgrade back on every PR, which is precisely what D16 exists to prevent. Projects already map 1:1 to CI matrix entries (`e2e/README.md`, and G4's D1 rests on the same property). No `bootstrap` dependency and no `storageState` for the same reason `pristine` has none: bootstrap targets whichever container is up *before* a project starts, and both specs replace or destroy a container mid-run — a persisted admin token would point at an instance that no longer exists. `retries: 0` because attempt 1 consumes the state attempt 2 needs, exactly as in `pristine`. "Runs alone" is already the rule for `pristine` and, per G4's D2, for `streaming-greybox`; in CI it is free (each matrix job has its own runner and container), and locally it is one line in `e2e/README.md`. It is also what makes a global `CoreSettings` mutation legitimate here. Rejected: a separately-named container/volume/port so a project could coexist with `seeded` locally — `destroy()` reaches the shared network and provider anyway, so coexistence would be an illusion with a worse failure mode |
...
| D10 | **The upgrade baseline is a fork image resolved from the CI event, not `:latest`.** `lifecycle-tests.yml`'s `upgrade-migrations` job — the only consumer — sets `DISPATCHARR_E2E_BASELINE_IMAGE` to `ghcr.io/d10scot/dispatcharr:${{ github.event.before }}` on a push and `:${{ github.event.pull_request.base.sha }}` on a `pull_request`. The fixture falls back to `:latest` whenever the resolved tag is **empty, all-zeroes, or does not resolve in the registry**, and says loudly which it used. The resolved digest is logged and attached to the report either way | Settled decision D-2 said "use `:latest`". This keeps its intent — *the fork's own upgrade path* — and closes a hole in its mechanism: **two workflows publish `:latest` on every push to main**, so `:latest` can be the commit under test, and the upgrade test would upgrade from itself and pass vacuously while looking perfectly green. Both event-derived SHAs name a commit that is on `main`, is an ancestor of the code under test, and is never that code — and `docker-build.yml` publishes a full-40-char-SHA tag for the commit it builds, which is exactly the tag being asked for. But that workflow is **path-filtered** (`apps/**`, `core/**`, `dispatcharr/**`, `frontend/**`, `docker/**`, `scripts/**`, `pyproject.toml`, `uv.lock`, `version.py`, `manage.py`, and the workflow file itself), not run on every push to main: a docs-only or workflow-only merge — this spec's own corrections among them — triggers no build and publishes no SHA tag at all. **Three fallback conditions, not one**, because neither field exists on the other's event type and *neither exists on `schedule` or `workflow_dispatch`* — two of this workflow's four triggers, so the empty case is a normal path rather than an edge, and it arrives as a variable that is set-but-invalid (`ghcr.io/d10scot/dispatcharr:`) rather than unset. All-zeroes covers a force-push or a first push; registry non-resolution is independent of both, and is not the edge case it might look like — it is the **normal** outcome for any push whose commit `docker-build.yml`'s path filter skipped, and it also covers a run that was triggered but failed. Rejected: (a) bare `:latest`, for the race above; (b) hand-pinning a historical SHA in the workflow, correct once and silently stale forever after. The `:latest` fallback is retained deliberately so a local run and a commit whose own build failed both still work, with the degradation announced rather than silent. **The all-zeroes case is branch creation or a force-push** — worth naming, so that a fallback firing on `main` is read as "no previous commit to resolve" rather than as a registry failure |
| D11 | **Both new specs assert Postgres-backed state only, by id, with recorded field values — never a count, never an unfiltered list** | Roadmap rule 4. Redis is excluded by construction, not by preference: AIO configures no persistence and `wait_for_redis.py` flushes on every boot, so a Redis-backed assertion would be asserting a falsehood. The concrete row set both specs create, through `Seeder`: a `Channel` (with an explicit `channel_number`), a `ChannelProfile`, a custom `StreamProfile` (`is_active: true`, a distinctive `parameters` string, and unlocked — so it is distinguishable from the built-in profiles by construction), an inactive `M3UAccount` on an unroutable URL, an inactive `EPGSource`, a `User` at `user_level: 1`, plus one PATCH into the `system_settings` `CoreSettings` row, setting **`max_system_events`** (an integer defaulting to 100, `CoreSettings.get_system_settings`) to an unmistakable value. That field is chosen because a PATCH there is inert: `CoreSettingsViewSet.update` special-cases only `STREAM_SETTINGS_KEY` (fires `rehash_streams.delay`) and `DVR_SETTINGS_KEY` (reschedules recordings), and `CoreSettingsSerializer.update` only `NETWORK_ACCESS_KEY` (CIDR validation) — `system_settings` trips none of them, and `max_system_events` only bounds `SystemEvent` retention. Three mechanics the implementer must not discover the hard way: the viewset has **no `lookup_field`**, so the row is addressed by pk — list, find the row whose `key` is `system_settings`, PATCH that id; `value` is the **whole group blob**, so this is read-modify-write or it silently drops `time_zone`, `preferred_region`, `auto_import_mapped_files`, `enable_ip_lookup` and `catchup_enabled`; and the assertion reads the row back **through the API**, never through `get_system_settings`, which merges defaults on read — masking a clobbered blob — and reads through the Redis-backed Django cache, which is flushed on every boot. Assertions after the lifecycle event, in order of what they prove: **(a)** the pre-event access token still authenticates via `/api/accounts/users/me/` returning `e2e-admin` — proving `/data/jwt` persisted, because a regenerated `DJANGO_SECRET_KEY` invalidates every token; **(b)** each of the seven rows `GET`s by id with the field values recorded at creation; **(c)** `max_system_events` reads back changed, not defaulted; **(d)** the lifecycle event demonstrably happened — `docker inspect`'s `.State.StartedAt` moved for the restart, and `.Image` equals the local image's id for the upgrade. **(d) is not padding**: without it a spec that silently failed to restart or replace anything passes every other assertion. Rejected: asserting through the UI, which adds browser flake to a test whose entire subject is the data layer |
...
| D13 | **Extract `provisionAdmin(request, baseURL)` from `bootstrap.setup.ts` into `e2e/setup/`, and have both callers use it. The lifecycle specs build `ApiClient` and `Seeder` directly from the pair it returns.** **Superseded by ruling G7-R4** (see "Rulings made at planning time" above): the shipped extraction is two functions, `ensureSuperuser` (probe/guard/create) plus `provisionAdmin` (that, then login), not the single function below — a single `provisionAdmin` would make `bootstrap.setup.ts` spend a login on every run against the suite-wide 3/minute budget | Both specs need the probe → `assertMayCreateSuperuser` → POST → `loginWithThrottleBackoff` sequence on an instance nothing has bootstrapped. Duplicating it would duplicate the superuser guard, which `superuser-guard.ts` states plainly is not a guard if only one of the two paths consults it. The extraction is deliberately minimal — the probe/create/login block only. `persistAdminAuth` and the `IntervalSchedule` pre-warm stay in `bootstrap.setup.ts`: the lifecycle specs hold the pair in memory and never write `playwright/.auth/`, and a single-worker spec creating M3U accounts serially cannot lose the concurrent-create race the pre-warm exists to prevent |
| D14 | **Two npm scripts, `test:lifecycle` and `test:lifecycle-upgrade`, and `lifecycle` added to `e2e-tests.yml`'s matrix in the same PR — the restart project only, per D16.** That job needs **no** inverse filter: the projects are disjoint by `testMatch` (D8), so `--project=lifecycle` selects the restart spec and cannot reach the upgrade spec. This is the whole reason the split is structural rather than a `--grep` pair | `e2e/README.md`: "If you add a fourth project to `playwright.config.ts`, add it to that matrix too — nothing wires new projects in automatically." One script per project keeps that mapping honest in both directions: each name corresponds to exactly one CI job, so nothing silently runs a spec the job it names was not budgeted for. `npm test` with no suffix already fails with a message listing the populations; both join that list, with the "run it alone" caveat attached |
...
| D16 | **The upgrade spec runs in `lifecycle-tests.yml`, not `e2e-tests.yml`. Only the ~3-minute restart spec joins the PR matrix** | Placing both lifecycle specs in `e2e-tests.yml` puts ~12 minutes on **every PR**, where the longest existing test job is `seeded` at 284s. That makes lifecycle the long pole and roughly doubles E2E latency on every PR — breaching the ~10-minute ceiling that G4 is being sharded into three projects specifically to protect. Sharding one goal to hold that budget while another breaches it is incoherent. The move costs nothing because `lifecycle-tests.yml` **already builds the AIO image** for the two bash suites (D6); the upgrade job is a third consumer of an artifact that already exists. Migration-touching PRs keep their gate through D4's narrow `pull_request` filter. **The trade, stated rather than implied: the upgrade spec is not self-guarding on PRs.** Its own machinery — `docker/entrypoint.sh`, `scripts/e2e_up.sh`, `e2e/fixtures/instance.ts` — is on the *push* filter but not the `pull_request` one, so a PR that breaks the spec without touching a model or a migration goes green and the breakage surfaces on `main`. That is accepted: those paths change rarely, and widening the PR filter to cover them would restore most of the ~12 minutes this decision removes **Rejected:** (a) both specs in `e2e-tests.yml`, for the ~12-minute PR cost above; (b) both in `lifecycle-tests.yml`, which loses cheap per-PR restart signal for no saving, since the restart spec is ~3 minutes and reuses a build that workflow already has; (c) a separate third workflow, paying a second 45-minute-budget AIO build to run one spec |

## Piece A — wire up the bash suites

**One new file: `.github/workflows/lifecycle-tests.yml`.** It carries the two bash suites and, under
D16, Piece B's upgrade spec — so it depends on Piece B's fixture and on `e2e/`'s dependencies, and
is not a self-contained change.

```
build (AIO + provider images, timeout 45)
  └─ artifact ──┬─→ puid-pgid          (docker tag → dispatcharr:puid-test, --skip-build)
                ├─→ tls-postgres       (docker tag → dispatcharr:tls-test,  --skip-build)
                └─→ upgrade-migrations (D16: Playwright, --project=lifecycle-upgrade)
```
...
- `concurrency: { group: lifecycle-tests-${{ github.workflow }}-${{ github.ref }},
  cancel-in-progress: ${{ github.event_name == 'pull_request' }} }` — the group naming matches
  every other workflow here; the conditional is the deliberate departure argued in D4.
- `build`: `timeout-minutes: 45`, `docker build -f docker/Dockerfile -t dispatcharr-lifecycle:local .`
  (**superseded by ruling G7-R6**: the shipped workflow tags this build `dispatcharr-e2e:local`
  directly, since that is the tag `scripts/e2e_up.sh` already defaults to — see the annotation on
  the `upgrade-migrations` bullet below),
  **plus `docker build -f e2e-upstream/Dockerfile -t dispatcharr-e2e-upstream:local e2e-upstream`**,
  both `docker save | gzip`'d into one archive and uploaded. Identical in intent to `e2e-tests.yml`'s
  `build`, provider image included. Neither lifecycle spec *uses* the provider — but `e2e_up.sh`
  starts it unconditionally and waits for `/scenarios` before starting Dispatcharr, and
  `instance.up()` sets `DISPATCHARR_E2E_SKIP_UPSTREAM_BUILD`, so without the image in the artifact
  the script dies at its `docker image inspect` of a tag nothing built. It is a hard startup
  dependency of the boot path, not a test dependency, which is exactly why it is easy to omit.
  Building it here rather than letting the script build it also preserves D6's one-artifact
  guarantee: `e2e-upstream`'s ffmpeg is deliberately unpinned, so two builds are not guaranteed to
...
- **The `upgrade-migrations` job (D16)** is the one job here that runs Playwright, which this
  workflow otherwise never does. Beyond the shared artifact it needs: `actions/setup-node` at the
  version `e2e-tests.yml` already uses, `npm ci` in `e2e/`, `npx playwright install --with-deps` for
  the one browser the project needs, `npm run typecheck` — every e2e job in `e2e-tests.yml` is gated
  on it, and a project that does not typecheck cannot be trusted to have driven Docker correctly —
  `E2E_BASE_URL: http://localhost:9191` set explicitly to match `e2e-tests.yml` rather than relying
  on `playwright.config.ts`'s default, and `DISPATCHARR_E2E_BASELINE_IMAGE` per D10.
  **Superseded by ruling G7-R6** (see "Rulings made at planning time" above): this paragraph has
  `build` produce `dispatcharr-lifecycle:local` and calls a `docker tag … dispatcharr-e2e:local` step
  here "critical", because D9's `recreate({ image: 'dispatcharr-e2e:local' })` names that tag and
  nothing else creates it. The shipped workflow instead builds `dispatcharr-e2e:local` directly in
  `build` — the tag `scripts/e2e_up.sh` already defaults to — so this job has no tag step at all;
  the two bash-suite jobs do their own `docker tag` into their own names. It then
  runs `npx playwright test --project=lifecycle-upgrade`. Its `timeout-minutes` is 45, matching the
  suite jobs: it pulls a ~3.6 GB baseline on top of the artifact load. Finally, an `if: always()`
  upload of `e2e/playwright-report/`, as `e2e-tests.yml` does — `playwright.config.ts` sets
  `trace: 'retain-on-failure'` and `screenshot: 'only-on-failure'`, so those artifacts exist and
  would otherwise be discarded with the runner; D7's tee'd logs cover the bash suites and nothing
  else covers this job.
  Omitting any of these is a silent failure of a different order — a missing `npm ci` fails loudly,
  but a missing `docker tag` fails as *"the upgrade ran and the data survived"* against the wrong
  image, and a missing baseline variable silently takes D10's `:latest` fallback.
...
**New directory `e2e/tests/lifecycle/`, two new projects, one new fixture, one script mode, one
extracted helper.** Both specs are self-contained: each brings up the instance it needs, provisions
its own admin, seeds, performs the lifecycle event, and asserts. Neither reads anything the other
wrote, which is what lets D16 send them to different workflows without a declared sequence.

```
e2e/tests/lifecycle/restart-persistence.spec.ts    ~3 min   reuses the container: --stop, then start
  → project `lifecycle`,          run by e2e-tests.yml's matrix
e2e/tests/lifecycle/upgrade-migrations.spec.ts     ~9 min   baseline image → --recreate → local image
  → project `lifecycle-upgrade`,  run by lifecycle-tests.yml
```

Both projects point `testDir` at the same directory and select their one spec with `testMatch`,
so the split is expressed in the config where CI can honour it rather than in a `--grep` on the
command line (D8).
...
Its file header states plainly that it drives Docker from inside a test, that only the two
lifecycle projects may import it, and why (D8). The upgrade spec's teardown is a `finally` calling `down()` —
...
| 3 | Restart preserves channels and settings | `e2e/tests/lifecycle/restart-persistence.spec.ts`, project `lifecycle`, run by `e2e-tests.yml`'s matrix | `instance.up()`, `provisionAdmin`, seed the seven-row set of D11, `instance.restart()`, then assertions (a)–(d) with `startedAt` as the event proof | ~3 min |
| 4 | Upgrade from previous release (migrations) | `e2e/tests/lifecycle/upgrade-migrations.spec.ts`, project `lifecycle-upgrade`, run by `lifecycle-tests.yml` (D16) | `instance.pull(baseline)` per D10 and attach the digest; `up({ image: baseline, reset: true })`; `provisionAdmin`; capture the applied-migration set; seed the same seven rows; `recreate({ image: 'dispatcharr-e2e:local' })`; assert D12's two migration checks plus (a)–(d) with `imageId` as the event proof; `finally` → `down()` | ~9 min |
...
| **Disk on the bash-suite runners.** A 3.6 GB image load, four-plus pulled images, and up to 20 PostgreSQL data volumes in one job | D5's parallel split is half the answer. The other half is not passing `--keep-on-fail`, which is what would let volumes accumulate across an entire failing run. Worth a dry run before trusting D4's timeout budget |
| **Disk on the `upgrade-migrations` runner is tighter still, and for a different reason.** That job loads the ~3.6 GB AIO artifact **and** pulls a baseline image of comparable size, then holds a PostgreSQL data volume across two container generations — where every other job in either workflow holds one image | Nothing in D5's split helps here: it is one job by construction. If it proves too tight, `docker image rm` the baseline immediately after `recreate()` — the spec has no further use for it once the swap is asserted — before reaching for a larger runner |
...
| **A `tee` without `pipefail` turns every failure green** | GitHub's default `run:` shell on Linux is `bash -e {0}` — `-e` but **no** `-o pipefail` — so the default is *not* safe; only an explicit `shell: bash` gets `-eo pipefail`. Set `shell: bash` explicitly on every piped step, with a comment saying why (D7; fixed in `lifecycle-tests.yml` at `8f355d6a` after shipping backwards) |
...
| **`docker/Dockerfile`'s lockfile-free unpinned `npm install`** means the image row 4 builds is not guaranteed identical across two runs of the same commit | Pre-existing, tracked debt in `CLAUDE.md`. Named here because row 4 is the one new test that always builds fresh rather than reusing a published image, so it inherits the nondeterminism directly |
| **A lifecycle project destroys containers other projects are using** | Each runs alone: its own job and runner in CI — and after D16 the two are not even in the same workflow — plus a documented rule locally, alongside the same rule that already exists for `pristine` and (per G4's D2) `streaming-greybox`. `e2e/README.md` and the `instance.ts` header both state it |
...
| **The `lifecycle` job could exceed `e2e-tests.yml`'s 30-minute job timeout** — two container boots on a cold runner | Comfortable after D16 moved the baseline pull and the third and fourth boots out: ~3 minutes against a 30-minute ceiling. If it ever tightens, raise the timeout on that job rather than trimming assertions; the AIO build is the long pole in that workflow regardless |
