...
- **The `upgrade-migrations` job (D16)** is the one job here that runs Playwright, which this
  ...
  on `playwright.config.ts`'s default, `DISPATCHARR_E2E_BASELINE_IMAGE` per D10, and — critically —
  `docker tag dispatcharr-lifecycle:local dispatcharr-e2e:local`, because D9's
  `recreate({ image: 'dispatcharr-e2e:local' })` names that tag and nothing else creates it. It then
