...
| D14 | **Two npm scripts, `test:lifecycle` and `test:lifecycle-upgrade`, and `lifecycle` added to `e2e-tests.yml`'s matrix in the same PR — the restart project only, per D16** | `e2e/README.md`: "If you add a fourth project to `playwright.config.ts`, add it to that matrix too — nothing wires new projects in automatically." One script per project keeps that mapping honest in both directions: each name corresponds to exactly one CI job, so nothing silently runs a spec the job it names was not budgeted for. `npm test` with no suffix already fails with a message listing the populations; both join that list, with the "run it alone" caveat attached |
