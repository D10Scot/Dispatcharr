...
- **The `upgrade-migrations` job (D16)** is the one job here that runs Playwright, which this
  workflow otherwise never does. It needs, beyond the shared artifact: `actions/setup-node` with
  the version `e2e-tests.yml` already uses, `npm ci` in `e2e/`, `npx playwright install --with-deps`
  for the one browser the project needs, and — critically — `docker tag dispatcharr-lifecycle:local
  dispatcharr-e2e:local`, because D9's `recreate({ image: 'dispatcharr-e2e:local' })` names that tag
  and nothing else creates it. It then runs `npx playwright test --project=lifecycle --grep upgrade`.
  Its `timeout-minutes` is 45, matching the suite jobs: it pulls a ~3.6 GB baseline on top of the
  artifact load. Omitting any of these is a silent failure — a missing `npm ci` fails loudly, but a
  missing `docker tag` fails as *"the upgrade ran and the data survived"* against the wrong image.
