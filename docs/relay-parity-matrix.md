...
| 8 | A new client joins roughly 5 seconds behind live, positioned through the `chunk_timestamps` sorted set rather than at the newest chunk | `apps/proxy/live_proxy/input/buffer.py:478-513`, `apps/proxy/live_proxy/output/ts/generator.py:255-270` | `apps/proxy/live_proxy/tests/test_relay_stream_switch.py::PositioningTests::test_a_new_client_starts_behind_live` | `new_client_behind_seconds` defaults to 5. Positioning happens once, at client setup; a separate, unrelated mechanism elsewhere in the same file recovers a client whose next expected chunk has already expired, and is not part of this behaviour. Reshaped after an elapsed-time version of this test passed against a genuine mechanism break -- reads in this harness are never throttled to production pacing, so a head-positioned client drains just as fast as a behind-positioned one (harness/README.md, "Three traps for the tests that come next"). The reshaped test asserts on the joining client's first delivered packet's position (a unique per-packet pid, `numbered_ts()`) against the already-connected client's own read position, and goes red under the same break |
