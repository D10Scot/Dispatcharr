...
| 8 | A new client joins roughly 5 seconds behind live, positioned through the `chunk_timestamps` sorted set rather than at the newest chunk | `apps/proxy/live_proxy/input/buffer.py:478-513`, `apps/proxy/live_proxy/output/ts/generator.py:255-270` | `owed: 2a-3` | `new_client_behind_seconds` defaults to 5. Positioning happens once, at client setup; a separate, unrelated mechanism elsewhere in the same file recovers a client whose next expected chunk has already expired, and is not part of this behaviour. `test_a_new_client_starts_behind_live` (PR #239) does not pin this row: a genuine mechanism break (forcing live-head positioning in `_setup_streaming` regardless of `new_client_behind_seconds`) left the test green, because reads in this harness are never throttled to real production pacing -- a head-positioned client drains just as fast as a behind-positioned one. The test stays in the tree pending a redesign; it is not deleted because it still asserts something true (a positioned client drains a real backlog fast), just not the thing this row claims |
