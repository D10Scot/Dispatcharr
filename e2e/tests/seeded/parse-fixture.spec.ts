...
test('parseM3u reads the header attributes', { tag: '@contract' }, () => {
...
test('parseM3u pairs each EXTINF with the URL beneath it', { tag: '@contract' }, () => {
  const playlist = parseM3u(PLAYLIST);
...
test('parseM3u keeps a comma inside the title', { tag: '@contract' }, () => {
...
test('parseM3u does not read attributes out of the title', { tag: '@contract' }, () => {
...
test('parseM3u reads an EXTINF line that carries no attributes', { tag: '@contract' }, () => {
...
test('parseM3u rejects a body that is not a playlist', { tag: '@contract' }, () => {
  expect(() => parseM3u('<html>nope</html>')).toThrow(/not an M3U playlist/);
});

test('parseM3u rejects an EXTINF with no URL beneath it', { tag: '@contract' }, () => {
  expect(() =>
    parseM3u('#EXTM3U\n#EXTINF:-1 tvg-id="1",A\n#EXTINF:-1 tvg-id="2",B\nhttp://h/2')
...
test('parseXmltv reads channels and decodes entities', { tag: '@contract' }, () => {
...
test('parseXmltv reads programmes with their channel and title', { tag: '@contract' }, () => {
  const guide = parseXmltv(GUIDE);
...
test('parseXmltv tolerates extra attributes on <channel> and <display-name>', { tag: '@contract' }, () => {
...
test('parseXmltv rejects a body that is not an XMLTV document', { tag: '@contract' }, () => {
...
test('expectWellFormedXml accepts valid XML', { tag: '@contract' }, async ({ adminPage }) => {
  await expectWellFormedXml(adminPage, GUIDE);
});

test('expectWellFormedXml rejects malformed XML', { tag: '@contract' }, async ({ adminPage }) => {
