...
| VOD | **Gap:** this build's `/movie/`/`/series/` routes (`e2e-upstream/src/xc/router.ts`) answer any HTTP method identically to GET — unlike the `/live/` route, which 405s a non-GET/HEAD method (`server.ts:375`). A `POST /movie/<user>/<pass>/<id>.<ext>` currently gets a 200 with the full body rather than a 405. Not fixed in G8 (unrequested scope for a wiring-only build); G9 decides whether the inconsistency is worth closing before writing tests against it | G9 | todo |
